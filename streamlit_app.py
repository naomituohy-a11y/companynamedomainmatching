...
def _clean_domain(domain: str) -> str:
    ...
    # keep second-level for fuzzy (e.g., "microsoft")
    parts = domain.split(".")
...
def compare_company_domain(company: str, domain: str):
    ...
    # if the 'domain' looks like an email, convert to domain
    d_email = _extract_domain_from_email(d_raw)
    d_raw = d_email or d_raw
    d = _clean_domain(d_raw)

    # exact or direct containment (after normalization)
...
# ------------------ UI ------------------
st.set_page_config(page_title="Company ↔ Domain Checker", page_icon="🧪", layout="wide")
...
st.markdown(
    ...
    <style>
      ...
      h1, h2, h3, h4, h5, p, label, span, div {color: #e9eef7;}
